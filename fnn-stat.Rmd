...
library(statnnet)
nn <- selectnn(medv ~ ., data = Boston, Q = 10, n_init = 10,
               maxit = 5000)
summary(nn)
```
...
cat(c("[...]", capture.output(summary(stnn))[12:25]
      , "[...]"), sep = "\n")  
```
...
Our package `statnnet` extends existing neural network packages
(currently `nnet`) to allow for a more interpretable output.


### References

<font size="5">Fukumizu, K. (1996).
A regularity condition of the information matrix of a multilayer perceptron
network. Neural Networks, 9(5):871–879.</font>   
<br>
<font size="5">James, G., Witten, D., Hastie, T., and Tibshirani, R. (2022).
ISLR2: Introduction to Statistical Learning, Second Edition.
R package version 1.3-1.</font>
